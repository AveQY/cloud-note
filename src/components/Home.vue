...
const handleMobileTocSelect = (id: string) => {
  showMobileToc.value = false
  setTimeout(() => scrollToHeading(id), 0)
}
...
const handleSelectNote = (note: Note) => {
  ...
  showSidebar.value = false
  router.push({ name: 'NoteView', params: { noteId: note.id } })
}
...
const handleBackToList = () => {
  // On mobile, use browser history to go back
  if (window.innerWidth <= 768) {
    router.back()
  } else {
    // On desktop, just reset the state
...
watch(() => route.params.noteId, async (noteId) => {
  if (noteId && !selectedNote.value) {
    ...
    // No noteId in route, reset state
    selectedNote.value = null
    showSidebar.value = true
  }
}, { immediate: true })
  ...
  <div class="home">
    <aside
      ...
      :class="{
        ...
        'home__sidebar--library': showSidebar && !selectedNote
      }"
    >
      <!-- Note list view -->
    ...
    <div v-if="showMobileToc" class="home__mobile-toc" role="dialog" aria-modal="true" aria-label="笔记目录">
      <button class="home__mobile-toc-overlay" type="button" aria-label="关闭目录" @click="handleCloseMobileToc"></button>
      <section class="home__mobile-toc-panel">
        <div class="home__mobile-toc-header">
          <span>目录</span>
          <button class="home__mobile-toc-close" type="button" aria-label="关闭目录" @click="handleCloseMobileToc">×</button>
        </div>
        <div class="home__mobile-toc-list">
          <div v-if="headings.length === 0" class="home__toc-empty">暂无目录</div>
          <template v-for="(heading, index) in headings" :key="index">
            <button
              v-if="heading.level <= 3"
              class="home__mobile-toc-item"
              :class="{
                'home__mobile-toc-item--h2': heading.level === 2,
                'home__mobile-toc-item--h3': heading.level === 3,
                'home__mobile-toc-item--active': activeHeadingId === heading.id
              }"
              type="button"
              @click="handleMobileTocSelect(heading.id)"
            >
              {{ heading.text }}
            </button>
          </template>
...
.home__sidebar.home__sidebar--library {
  width: 100%;
  max-width: none;
  flex: 1 1 100%;
  border-right: 0;
...
.home__sidebar.home__sidebar--library > .home__sidebar-header {
  display: none;
}

.home__sidebar.home__sidebar--library > .note-list {
  min-height: 0;
}
...
@media (max-width: 768px) {
  ...
  .home__mobile-toc-item--h2 {
    padding-left: 24px;
  }

  .home__mobile-toc-item--h3 {
    padding-left: 40px;
    color: #6b7280;
    font-size: 13px;
  }

  .home__mobile-toc-item--active {
    border-left-color: var(--primary-color);
    background: #f0f7ff;
    color: var(--primary-color);
    font-weight: 500;
