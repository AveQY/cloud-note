...
const metadataPath = join(__dirname, 'log', 'note-metadata.json')

const defaultMetadata = () => ({ categories: [], notes: {} })

const loadMetadata = () => {
  try {
    if (!existsSync(metadataPath)) return defaultMetadata()
    ...
    console.error('读取笔记分类元数据失败:', error)
...
const safeFilenameFromPath = (filePath = '') => decodeURIComponent(filePath.replace(/^\/file\//, ''))
  ...
  const files = readdirSync(fileDir)
    ...
    .map(filename => {
      ...
      return {
        ...
        categoryId: metadata.notes[filename]?.categoryId || null
      }
    })
...
app.get('/api/categories', (req, res) => {
  const metadata = loadMetadata()
  const counts = loadFileList().reduce((result, note) => {
    const key = note.categoryId || 'uncategorized'
    result[key] = (result[key] || 0) + 1
    return result
  }, {})
  res.json({ categories: metadata.categories, counts })
})
...
app.post('/api/categories', (req, res) => {
  try {
    ...
    const metadata = loadMetadata()
    if (metadata.categories.some(item => item.name.toLowerCase() === name.toLowerCase())) return res.status(400).json({ error: '分类名称已存在' })
    const category = { id: `category-${Date.now()}`, name, createdAt: Date.now() }
...
app.post('/api/categories/rename', (req, res) => {
  try {
    ...
    const name = String(req.body.name || '').trim()
    const metadata = loadMetadata()
    const category = metadata.categories.find(item => item.id === id)
    if (!category || !name) return res.status(400).json({ error: '分类不存在或名称为空' })
    if (metadata.categories.some(item => item.id !== id && item.name.toLowerCase() === name.toLowerCase())) return res.status(400).json({ error: '分类名称已存在' })
    category.name = name
    saveMetadata(metadata)
    res.json({ success: true, category })
  } catch { res.status(500).json({ error: '重命名分类失败' }) }
})
...
app.post('/api/categories/delete', (req, res) => {
  try {
    const { id } = req.body
    const metadata = loadMetadata()
    metadata.categories = metadata.categories.filter(item => item.id !== id)
    Object.values(metadata.notes).forEach(note => { if (note.categoryId === id) note.categoryId = null })
    saveMetadata(metadata)
    res.json({ success: true })
  } catch { res.status(500).json({ error: '删除分类失败' }) }
})

app.post('/api/note-category', (req, res) => {
  try {
    const filename = safeFilenameFromPath(req.body.path)
    const categoryId = req.body.categoryId || null
    if (!filename || !existsSync(join(__dirname, 'file', filename))) return res.status(404).json({ error: '笔记不存在' })
    const metadata = loadMetadata()
    if (categoryId && !metadata.categories.some(item => item.id === categoryId)) return res.status(400).json({ error: '分类不存在' })
    metadata.notes[filename] = { ...(metadata.notes[filename] || {}), categoryId }
    saveMetadata(metadata)
    res.json({ success: true, categoryId })
  } catch { res.status(500).json({ error: '移动分类失败' }) }
})
...
app.post('/api/delete', (req, res) => {
  try {
    ...
    unlinkSync(fullPath)
    const metadata = loadMetadata()
    delete metadata.notes[filename]
    saveMetadata(metadata)
...
app.post('/api/create', (req, res) => {
  try {
    ...
    if (categoryId) {
      const metadata = loadMetadata()
      metadata.notes[filename] = { categoryId }
      saveMetadata(metadata)
    }
...
app.post('/api/rename', (req, res) => {
  try {
    ...
    const metadata = loadMetadata()
    if (metadata.notes[filename]) {
      metadata.notes[newFilename] = metadata.notes[filename]
      delete metadata.notes[filename]
      saveMetadata(metadata)
    }
  ...
  try {
    ...
    req.on('end', () => {
      try {
        ...
        const fileDir = join(__dirname, 'file')
        const filePath = join(fileDir, filename)
        const overwritePart = body.split(`--${boundary}`).find((part) => part.includes('name="overwrite"')) || ''
        const overwrite = overwritePart.replace(/\r/g, '').includes('\n\ntrue')

        if (existsSync(filePath) && !overwrite) {
          return res.status(409).json({ error: '文件已存在', code: 'NOTE_CONFLICT', conflicts: [filename] })
        }
...
app.get('/api/backup', (req, res) => {
  try {
    ...
    if (type === 'notes' || type === 'all') {
      ...
      // 笔记备份必须同时包含分类及笔记-分类关联信息。
      if (type === 'notes' && existsSync(metadataPath)) {
        zip.file(metadataPath, { name: 'note-metadata.json' })
      }
    }
...
app.post('/api/restore', restoreUpload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const overwrite = String(req.query.overwrite || req.body?.overwrite || '').toLowerCase() === 'true'
    const extractedNoteNames = existsSync(extractedFileDir)
      ? readdirSync(extractedFileDir).filter((name) => statSync(join(extractedFileDir, name)).isFile())
      : readdirSync(tempExtractDir).filter((name) => name.endsWith('.md') && statSync(join(tempExtractDir, name)).isFile())

    if (!overwrite) {
      const conflicts = extractedNoteNames.filter((name) => existsSync(join(fileDirForBackup, name)))
      if (conflicts.length > 0) {
        ...
        return res.status(409).json({
          ...
          code: 'NOTE_CONFLICT',
          conflicts,
          message: `发现 ${conflicts.length} 个同名笔记`
        })
      }
    }
    ...
    const extractedMetadataPaths = [
      join(tempExtractDir, 'log', 'note-metadata.json'),
      join(tempExtractDir, 'note-metadata.json')
    ]
    const extractedMetadataPath = extractedMetadataPaths.find((candidate) => existsSync(candidate))

    if (extractedMetadataPath) {
      try {
        const restoredMetadata = JSON.parse(readFileSync(extractedMetadataPath, 'utf-8'))
        if (restoredMetadata && typeof restoredMetadata === 'object') {
          saveMetadata({
            categories: Array.isArray(restoredMetadata.categories) ? restoredMetadata.categories : [],
            notes: restoredMetadata.notes && typeof restoredMetadata.notes === 'object' ? restoredMetadata.notes : {}
          })
        }
      } catch {
        return res.status(400).json({ success: false, message: '分类数据损坏，无法恢复' })
      }
    }
